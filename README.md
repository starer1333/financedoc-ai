...
系统默认支持 demo mode，不依赖本地 Ollama，也不强制调用付费 LLM API，因此可以部署到 Streamlit Community Cloud。配置 API key 后，可切换到 OpenAI、DeepSeek 或 Kimi 进行真实 LLM 问答和 Multi-Agent 分析。
...
- Q&A Assistant 支持 Demo Mode、OpenAI、DeepSeek 和 Kimi
...
- API 状态：展示 Demo Mode / OpenAI / DeepSeek / Kimi 是否已配置
...
```text
AI 模式选择 OpenAI、DeepSeek 或 Kimi
